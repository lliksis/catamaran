<script lang="ts">
	...
	import type { IBountyStore } from '$lib/api/utils';
	import * as vendorConf from '$lib/vendorConf.json';
	import type { IBounty, IVendor } from '$lib/api/destiny2';
	import Bounty from './Bounty.svelte';
	import Icon from './Icon.svelte';
	import type { ICharacterContext } from '$lib/api/utils/types';
	...
	const backgroundColor = vendorConf.vendorBackgrounds[vendor.vendorHash] || 'grey';
	...
	const hasNoProgressIcon = vendorConf.noProgressIcon.includes(vendor.vendorHash);
	...
	$: isDisabled = (bounty: IBounty) => {
		return $store.some((b) => b.hash === bounty.hash) || items.some((b) => b.hash === bounty.hash);
	};
</script>
...
{#if !ignore}
	...
	>
		<div class="icon">
			{#if hasIcon}
				<Icon {vendorIcon} {progressIcon} progression={vendor.progression} />
			{/if}
		</div>
		...
		<div class="bounties">
			{#each vendor.bounties as bounty}
				<Bounty {bounty} actions={createActions(bounty)} disabled={isDisabled(bounty)} />
			{/each}
		</div>
	</div>
{/if}
