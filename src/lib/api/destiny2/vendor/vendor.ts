...
import type { IBountyObjective, IVendor, IBounty, IVendorProgression } from './vendor.types';
...
export const resolveVendors = (
	...
	for (const vendorHash in vendors.data) {
		...
		if (
			...
			const vendorDisplayProperties = DestinyVendorDefinition[vendor.vendorHash].displayProperties;
			...
			if (vendor.progression) {
				...
				const progression = DestinyProgressionDefinition[progressionHash];
				vendorProgression = {
					...vendor.progression,
			...
			const vendorGroup = DestinyVendorGroupDefinition[groupHash]?.categoryName;
			...
			for (const saleHash in items) {
				...
				if (item.itemType === DestinyItemType.Bounty) {
					const itemObjectives = itemComponents[vendorHash].objectives.data[saleHash].objectives;
					const objectiveProgress: IBountyObjective[] = itemObjectives.map((obj) => {
						return {
							progress: 0,
							completionValue: obj.completionValue,
							objectiveProgressDescription:
								DestinyObjectiveDefinition[obj.objectiveHash].progressDescription
						};
					});
					resolvedItems.push({
						...item,
